...
#SBATCH --output=realdata_rl_%j.out
#SBATCH --error=realdata_rl_%j.err
#
# Optional RL fine-tune (GSM8K) on top of an SFT checkpoint.
...
mkdir -p "$WANDB_DIR"
WANDB_RUN_BASE="${WANDB_RUN:-realdata_rl}"
...
# Checkpoint selection (from SFT)
DEPTH="${DEPTH:-4}"
MODEL_TAG="${MODEL_TAG:-d$DEPTH}"   # matches chatsft_checkpoints directory name
STEP_OPT=()
if [ -n "${STEP:-}" ]; then
  STEP_OPT=(--step="$STEP")
fi
...
echo ""
echo "Run config:"
echo "  MODEL_TAG=$MODEL_TAG STEP=${STEP:-latest} SOURCE=sft"
echo ""

torchrun --standalone --nproc_per_node="$NGPUS" -m scripts.chat_rl -- \
  --run="${WANDB_RUN_BASE}_rl" \
  --source="sft" \
  --model_tag="$MODEL_TAG" \
  "${STEP_OPT[@]}"
